...
class ExperimentRunner:
    ...
    def run(
        ...
        if not validate_experiment_function(experiment_func):
            raise ValueError(
                f"Function {experiment_func.__name__} is not a valid experiment"
            )

        meta = get_experiment_metadata(experiment_func)
        if not meta:
            raise ValueError(
                f"No experiment metadata found for {experiment_func.__name__}"
            )
        ...
        try:
            ...
            if self.auto_seed:
                ...
                if meta.seeds:
                    ...
                    experiment_metadata.random_seed = processed_params.get(
                        meta.seeds[0]
                    )
    ...
    def _process_results(
        self, experiment_metadata: ExperimentMetadata, meta, result: Any
    ) -> None:
                ...
                if artifact_name in result:
                    ...
                    try:
                        artifact_path = self.artifact_manager.store_artifact(
                            experiment_metadata.run_id, artifact_name, artifact_data
                        )
