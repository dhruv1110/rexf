...
from .interfaces import ExperimentMetadata, ExportInterface
...
class JSONExporter(ExportInterface):
    ...
    def export_experiment(
        self, metadata: ExperimentMetadata, include_artifacts: bool = False
    ) -> str:
    ...
    def export_experiments(
        self, experiments: List[ExperimentMetadata], include_artifacts: bool = False
    ) -> str:
        ...
        data = {
            ...
            "experiments": [
                self._metadata_to_dict(exp, include_artifacts) for exp in experiments
            ],
        }
        return json.dumps(data, indent=self.indent, default=self._json_serializer)
...
class ExperimentExporter:
    ...
    def export_to_file(
        ...
        if format == "json":
            if len(experiments) == 1:
                data = self.json_exporter.export_experiment(
                    experiments[0], include_artifacts
                )
            else:
                data = self.json_exporter.export_experiments(
                    experiments, include_artifacts
                )
            self.json_exporter.save_export(data, filepath)

        elif format == "yaml":
            if len(experiments) == 1:
                data = self.yaml_exporter.export_experiment(
                    experiments[0], include_artifacts
                )
            else:
                data = self.yaml_exporter.export_experiments(
                    experiments, include_artifacts
                )
    ...
    def export_comparison(
        ...
        if format == "json":
            ...
            with open(filepath, "w") as f:
                yaml.dump(
                    comparison_data, f, default_flow_style=False, allow_unicode=True
                )
        else:
            raise ValueError(f"Unsupported export format: {format}")
