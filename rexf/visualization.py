...
from .interfaces import VisualizationInterface, ExperimentMetadata
...
class ExperimentVisualizer(VisualizationInterface):
    ...
    def plot_metrics(
        ...
        for i, metric_name in enumerate(metric_names):
            ...
            if values:
                bars = ax.bar(range(len(values)), values, color=self.colors[:len(values)])
    ...
    def plot_parameter_space(
        ...
        if n_params == 1:
            ...
            fig, axes = plt.subplots(n_params, n_params, figsize=(self.figsize[0], self.figsize[1]))
        ...
        for i, param_x in enumerate(param_names):
            for j, param_y in enumerate(param_names):
                ...
                if i == j:
                    ...
                    values = [v for v in param_data[param_x] if v is not None and isinstance(v, (int, float))]
                    if values:
                        ax.hist(values, bins=min(10, len(set(values))), alpha=0.7, color=self.colors[0])
    ...
    def plot_experiment_timeline(
        ...
        if Rectangle:
            legend_elements = [
                Rectangle((0, 0), 1, 1, facecolor='green', alpha=0.7, label='Completed'),
    ...
    def create_comparison_table(
        ...
        for exp in experiments:
            row = {
                ...
                'status': exp.status,
                'duration_seconds': (exp.end_time - exp.start_time).total_seconds() if exp.end_time else None,
    ...
    def create_dashboard(
        ...
        sorted_experiments = sorted(experiments, key=lambda x: x.start_time)
        y_pos = range(len(sorted_experiments))
        durations = [(exp.end_time - exp.start_time).total_seconds() if exp.end_time else 0 
                    for exp in sorted_experiments]
        colors = ['green' if exp.status == 'completed' else 'red' if exp.status == 'failed' else 'orange' 
                 for exp in sorted_experiments]
        
        bars = ax1.barh(y_pos, durations, color=colors, alpha=0.7)
        labels = [f"{exp.name[:20]}..." if len(exp.name) > 20 else exp.name for exp in sorted_experiments]
        ...
        if all_metrics:
            ...
            values = [exp.metrics.get(metric_name, 0) for exp in experiments 
                     if metric_name in exp.metrics and isinstance(exp.metrics[metric_name], (int, float))]
            if values:
