...
class TestCLIAnalytics:
    ...
    def test_cli_with_no_database(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "rexf.cli.analytics", "--database", "nonexistent.db", "--summary"],
            capture_output=True,
            text=True,
        )
    ...
    def test_cli_summary_with_data(self, unique_db_path, sample_ml_experiment):
        ...
        try:
            ...
            # Test CLI summary
            result = subprocess.run(
                [sys.executable, "-m", "rexf.cli.analytics", "--database", unique_db_path, "--summary"],
                capture_output=True,
                text=True,
            )
    ...
    def test_cli_list_experiments(self, unique_db_path, sample_math_experiment):
        ...
        try:
            ...
            # Test CLI list
            result = subprocess.run(
                [sys.executable, "-m", "rexf.cli.analytics", "--database", unique_db_path, "--list"],
                capture_output=True,
                text=True,
            )
    ...
    def test_cli_json_output(self, unique_db_path, sample_math_experiment):
        ...
        try:
            ...
            # Test JSON format
            result = subprocess.run(
                [sys.executable, "-m", "rexf.cli.analytics", "--database", unique_db_path, "--list", "--format", "json"],
                capture_output=True,
                text=True,
            )
    ...
    def test_cli_csv_output(self, unique_db_path, sample_math_experiment):
        ...
        try:
            ...
            # Test CSV format
            result = subprocess.run(
                [sys.executable, "-m", "rexf.cli.analytics", "--database", unique_db_path, "--list", "--format", "csv"],
                capture_output=True,
                text=True,
            )
            
            assert result.returncode == 0
            assert "run_id,experiment_name,status,duration,parameters,metrics" in result.stdout
            assert "sample_math_experiment" in result.stdout
    ...
    def test_cli_insights(self, unique_db_path, sample_ml_experiment):
        ...
        try:
            ...
            # Test insights
            result = subprocess.run(
                [sys.executable, "-m", "rexf.cli.analytics", "--database", unique_db_path, "--insights"],
                capture_output=True,
                text=True,
            )
    ...
    def test_cli_filtering(self, unique_db_path, sample_ml_experiment, sample_math_experiment):
        ...
        try:
            ...
            result = subprocess.run(
                [sys.executable, "-m", "rexf.cli.analytics", "--database", unique_db_path, 
                 "--list", "--experiment-name", "sample_ml_experiment"],
                capture_output=True,
                text=True,
            )
            
            assert result.returncode == 0
            assert "sample_ml_experiment" in result.stdout
    ...
    def test_cli_output_file(self, unique_db_path, sample_math_experiment, temp_dir):
        ...
        try:
            ...
            result = subprocess.run([
                sys.executable, "-m", "rexf.cli.analytics",
                "--database", unique_db_path,
                "--summary",
...
class TestCLIIntegration:
    ...
    def test_cli_query_integration(self, unique_db_path, sample_ml_experiment):
        ...
        try:
            ...
            run.single(sample_ml_experiment, learning_rate=0.001, epochs=5)  # High accuracy
            run.single(sample_ml_experiment, learning_rate=0.1, epochs=5)    # Low accuracy
    ...
    def test_cli_compare_integration(self, unique_db_path, sample_optimization_experiment):
        ...
        try:
            ...
            run.single(sample_optimization_experiment, algorithm="newton", step_size=0.05)
